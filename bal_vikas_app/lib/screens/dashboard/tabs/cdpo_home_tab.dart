...
/// Dashboard for CDPO, CW, and EO roles (project-level view)
class CDPOHomeTab extends ConsumerWidget {
  const CDPOHomeTab({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final projectId = ref.watch(effectiveProjectIdProvider);
    ...
    if (projectId == null) {
      return Center(
        child: Padding(
          ...
          child: Text(
            isTelugu
                ? 'మీ ఖాతాకు ప్రాజెక్ట్ కేటాయించబడలేదు.\nనిర్వాహకుడిని సంప్రదించండి.'
    ...
    return statsAsync.when(
      ...
      data: (stats) => _buildDashboard(context, user, isTelugu, stats, projectId, isOverride),
    );
  }

  Widget _buildDashboard(
    BuildContext context, dynamic user, bool isTelugu, Map<String, dynamic> stats, int projectId, bool isOverride,
  ) {
    final totalChildren = stats['total_children'] ?? 0;
    final totalAwcs = stats['total_awcs'] ?? 0;
    final totalSectors = stats['total_sectors'] ?? 0;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _CDPOWelcomeCard(user: user, isTelugu: isTelugu),
          const SizedBox(height: 24),

          Text(isTelugu ? 'ప్రాజెక్ట్ విహంగావలోకనం' : 'Project Overview',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: DashboardStatCard(icon: Icons.location_city, value: '$totalSectors',
              label: isTelugu ? 'మొత్తం సెక్టార్లు' : 'Total Sectors', color: AppColors.primary,
              onTap: () => Navigator.push(context, MaterialPageRoute(
                builder: (_) => ScopedUnitListScreen(unitType: 'sector', scopeLevel: 'project', scopeId: projectId,
                  title: isTelugu ? 'సెక్టార్లు' : 'Sectors', subUnits: subUnits))))),
            ...
            Expanded(child: DashboardStatCard(icon: Icons.location_on, value: '$totalAwcs',
              ...
              onTap: () => Navigator.push(context, MaterialPageRoute(
                builder: (_) => ScopedUnitListScreen(unitType: 'awc', scopeLevel: 'project', scopeId: projectId,
          ...
          Row(children: [
            Expanded(child: DashboardStatCard(icon: Icons.people, value: '$totalChildren',
              ...
              onTap: () => Navigator.push(context, MaterialPageRoute(
                builder: (_) => ScopedChildrenListScreen(scopeLevel: 'project', scopeId: projectId,
          ...
          _EarlyWarningSection(isTelugu: isTelugu),
          const SizedBox(height: 24),
          ...
          // Sector-wise Performance — tap to drill into sector dashboard
          if (subUnits.isNotEmpty) ...[
            Text(isTelugu ? 'సెక్టార్ల వారీ పనితీరు' : 'Sector-wise Performance',
                style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            ...subUnits.map((unit) {
              ...
              return DashboardUnitCard(
                name: unitName,
                subUnitLabel: isTelugu ? 'AWCలు' : 'AWCs',
                ...
                onTap: () {
                  final sectorId = u['id'] as int?;
                  if (sectorId != null) {
                    Navigator.push(context, MaterialPageRoute(
                      builder: (_) => ScopedDashboardScreen(
                        scopeLevel: 'sector', scopeId: sectorId, scopeName: unitName)));
                  }
                },
              );
            }),
...
/// Early Warning section — shows children who are LOW/MEDIUM risk but predicted to worsen
class _EarlyWarningSection extends StatelessWidget {
  final bool isTelugu;
  const _EarlyWarningSection({required this.isTelugu});
  ...
  static Future<List<Map<String, dynamic>>> _loadEarlyWarningChildren() async {
    ...
    try {
      ...
      // Get latest result per child (highest id wins)
...
// Welcome card — only used in the native CDPO tab
class _CDPOWelcomeCard extends StatelessWidget {
  final dynamic user;
  final bool isTelugu;
  const _CDPOWelcomeCard({required this.user, required this.isTelugu});
